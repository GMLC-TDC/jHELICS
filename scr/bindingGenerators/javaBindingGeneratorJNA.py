...
class JavaBindingGenerator(object):
    ...
    def generateSource(self):
        ...
        """
        def createBoilerPlates(headerFiles: List[str], helicsElementMapTuples) -> List[str]:
            boilerPlateList = []
            copyRightStr = "/*\n"
            ...
            javaBoilerPlateStr += "static {\n\t\t"
            javaBoilerPlateStr += "System.loadLibrary(helicsJava);\n\t}"
            boilerPlateList.append(javaBoilerPlateStr)
            jvJNIBoilerPlateStr = copyRightStr
            jvJNIBoilerPlateStr += "#include <jni.h\n"
            jvJNIBoilerPlateStr += "/* Header for class JavaHELICS */\n"
            jvJNIBoilerPlateStr += "#ifndef _Included_com_java_helics_JavaHELICS\n"
            jvJNIBoilerPlateStr += "#define _Included_com_java_helics_JavaHELICS\n"
            jvJNIBoilerPlateStr += "#ifdef __cplusplus\n"
            jvJNIBoilerPlateStr += "extern \"C\" {"
            jvJNIBoilerPlateStr += "#endif"
            boilerPlateList.append(jvJNIBoilerPlateStr)
            jvCPPBoilerPlateStr = copyRightStr
            for h in headerFiles:
                jvCPPBoilerPlateStr += f"#include \"helics/helics.h\"\n"
            jvCPPBoilerPlateStr += "#include \"com_java_helics_JavaHELICS.h\"\n"
            jvCPPBoilerPlateStr += "#include <jni.h>\n"
            jvCPPBoilerPlateStr += "#include <stdlib.h>\n"
            jvCPPBoilerPlateStr += "#include <string>\n"
            boilerPlateList.append(jvCPPBoilerPlateStr)
        ...
        def createEnum(enumDict: dict()) -> None:
            ...
            with open(f"javaBindings/+helics/{enumSpelling}.java","w") as enumMFile:
                enumMFile.write("/*\n"+f"{enumComment}\n\nAttributes:")
                docStrBody = ""
                enumStrBody = ""
                ...
                enumMFile.write(f"\n\t{enumSpelling}(int value)"+"{"+"n\t\tthis.value = value;\n\t}")
                enumMFile.write(f"\n\tpublic int value()"+"{"+"n\t\treturn this.value;\n\t}")
                enumMFile.write("\n\}")
        ...
        def createFunction(functionDict: dict(), cursorIdx: int):
            ...
            if functionName not in functionsToIgnore:
                ...
                functionJavaCallStr += f"public public final static native" {functionName}(int resc, mxArray *resv[], int argc, const mxArray *argv[])" + "{\n"
        ...
        def getFunctionReturnType(functionDict: dict()) -> List[str]:
        ...
        if not os.path.exists(os.path.join(self.__rootDir, "JavaBindings/+helics")):
            os.makedirs(os.path.join(self.__rootDir, "JavaBindings/+helics"))
        else:
            try:
                shutil.rmtree(os.path.join(self.__rootDir, 'javaBindings'))
                os.makedirs(os.path.join(self.__rootDir, "javaBindings/+helics"))
            except:
        ...
        helicsJNIStr = ""
        helicsJAVAcppStr = ""
        helicsFilesBoilerCode = List[str]
        ...
        for cu in self.__helicsParser.parsedInfo.keys():
            ...
            if self.__helicsParser.parsedInfo[cu]["kind"] == cidx.CursorKind.MACRO_DEFINITION.name:
                macroMexWrapperFunctionStr, macroMexMainFunctionElementStr = createMacro(self.__helicsParser.parsedInfo[cu],int(cu))
            ...
            if self.__helicsParser.parsedInfo[cu]["kind"] == cidx.CursorKind.FUNCTION_DECL.name:
                functionMexWrapperFunctionStr, functionMexMainFunctionElementStr = createFunction(self.__helicsParser.parsedInfo[cu],int(cu))
        ...
        helicsFilesBoilerCode = createBoilerPlates(self.__helicsParser.headerFiles)
        helicsJAVAStr += helicsFilesBoilerCode[0]
        helicsJNIStr += helicsFilesBoilerCode[1]
        helicsJAVAcppStr += helicsFilesBoilerCode[2]
